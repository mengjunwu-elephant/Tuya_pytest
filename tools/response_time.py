...
robot = TuyaRobot(
    upper_ip="192.168.0.232",
    upper_port=6500,
    chassis_port='/dev/ttyCH341USB0',
    head_ip='192.168.0.231',
    head_port=6501,
    head_auto_connect=True,
...
def measure_time(func, *args, times=1000):
    ...
    for i in range(times):
        ...
        # time.sleep(0.1)
        # 按 TuyaRobot SDK：失败返回 CommandResult(ok=False)；
        # 超时/空返回计入丢包，其余业务失败计入错误（与老化网关一致）
        if result is None:
            packet_losses += 1
...
if __name__ == '__main__':
    stats = measure_time(robot.get_upper_angles)
