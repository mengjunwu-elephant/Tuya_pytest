...
- 刷新模式 `1` 不支持 Jog；`upper_jog_angle` 只保留一条返回 `-1` 的验证，结束后恢复双臂插补模式并回零。
...
- `upper_jog_coord` 单臂正常用例覆盖六轴正负向，双臂正常用例只覆盖 Z 轴正负向；单臂通过 `device.move_to_coord_initial_pose(arm_side)` 只移动目标臂，双臂通过无参调用移动双臂。持续 Jog 到状态码 `32`“坐标无解”，读取停止坐标后在 `finally` 中回零，不按坐标软件限位断言。
...
- 公共恢复/回零动作使用 `TuyaRobotBase.speed`，当前值为 `20`；不得在测试文件中重复硬编码恢复速度。
- 双臂整臂回零统一调用 `TuyaRobotBase.go_zero()`；设置参数后的公共默认恢复动作统一封装到 `TuyaRobotBase`，测试函数或 teardown fixture 只负责决定恢复时机。
...
- `send_upper_angle` 的 Excel 行使用 `arm_side`、`fresh_mode` 和 `motion_mode` 明确手臂、刷新/插补模式及单/双臂场景。刷新模式为 `1`，插补模式为 `0`；模式 setter 只作为前置条件，`expect_data` 仍只断言角度发送接口的业务数据。
...
4. 更新规则和技能中的收集基线；当前基线为 119 个测试文件、142 个测试函数、824 条参数化测试。
