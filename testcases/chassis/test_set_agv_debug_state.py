...
cases = get_test_data_from_excel(TuyaRobotBase.CHASSIS_TEST_DATA_FILE, 'set_agv_debug_state')
...
def test_set_agv_debug_state(chassis, case):
    title = case['title']
    ...
    with allure.step('调用 get_agv_debug_state 接口'):
        original = chassis.get_agv_debug_state()
        logger.debug(f"接口 get_agv_debug_state 返回：{original}")
    ...
    try:
        with allure.step('调用 set_agv_debug_state 接口'):
            chassis.set_agv_debug_state(case['state'])
        with allure.step('调用 get_agv_debug_state 接口'):
            actual = chassis.get_agv_debug_state()
            logger.debug(f"接口 get_agv_debug_state 返回：{actual}")
        with allure.step("断言接口返回结果"):
        ...
        with allure.step('调用 set_agv_debug_state 接口'):
            chassis.set_agv_debug_state(original)
