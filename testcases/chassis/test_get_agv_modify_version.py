...
@allure.feature('底盘')
@allure.story('底盘接口验证：get_agv_modify_version')
@pytest.mark.chassis
@pytest.mark.smoke
...
def test_get_agv_modify_version(chassis, case):
    ...
    with allure.step('调用 get_agv_modify_version 接口'):
        ...
        logger.debug(f"接口 get_agv_modify_version 返回：{response_1}")
    ...
    with allure.step("断言接口返回结果"):
        ...
        allure.attach(str(response_1), name="实际值", attachment_type=allure.attachment_type.TEXT)
        assert float(response_1) == float(expected), '版本号与 Excel 期望值不一致'
