...
@allure.feature('底盘')
@allure.story('底盘接口验证：set_agv_led_color')
@pytest.mark.chassis
@pytest.mark.manual
...
def test_set_agv_led_color(chassis, case):
    ...
    prompt_continue('确认允许本用例结束后将底盘 LED 恢复为绿色。', title='底盘 LED 状态确认')
    try:
        ...
        with allure.step("断言接口返回结果"):
            assert TuyaRobotBase.result_data(response) == 1, f'LED 颜色设置接口业务返回错误: {response!r}'
    finally:
        with allure.step('调用 set_agv_led_color 接口'):
            restore_response = chassis.set_agv_led_color(0, 255, 0, case['brightness'])
            assert TuyaRobotBase.result_data(restore_response) == 1, f'LED 颜色恢复失败: {restore_response!r}'
