# -*- coding: utf-8 -*-
import time
import allure
import pytest
from common1.test_data_handler import get_test_data_from_excel
from settings import TuyaRobotBase
from common1 import logger
cases = get_test_data_from_excel(TuyaRobotBase.CHASSIS_TEST_DATA_FILE, 'agv_wheel_stop')

@allure.feature('Chassis')
@allure.story('agv_wheel_stop')
@pytest.mark.chassis
@pytest.mark.motion
@pytest.mark.danger
@pytest.mark.parametrize('case', cases, ids=lambda case: case['title'])
def test_agv_wheel_stop(chassis, case):
    title = case['title']
    logger.info(f'》》》》》用例【{title}】开始测试《《《《《')
    with allure.step('检查底盘上电状态'):
        power_state = chassis.is_agv_powered_on()
        assert power_state == 0, f'底盘未处于正常上电状态: {power_state!r}'
    try:
        with allure.step('调用 agv_wheel_control 接口'):
            chassis.agv_wheel_control(case['forward_mps'], 0.0)
        time.sleep(case['duration'])
        with allure.step('调用 agv_wheel_stop 接口'):
            response = chassis.agv_wheel_stop()
            logger.debug('接口 agv_wheel_stop 返回：%r', response)
        assert response is not None, '轮毂停止接口未返回 ACK'
    finally:
        with allure.step('调用 agv_wheel_stop 接口'):
            chassis.agv_wheel_stop()
    logger.info(f'✅ 用例【{title}】测试通过')
    logger.info(f'》》》》》用例【{title}】测试完成《《《《《')
