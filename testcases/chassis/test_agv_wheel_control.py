...
@allure.feature('底盘')
@allure.story('底盘接口验证：agv_wheel_control')
@pytest.mark.chassis
@pytest.mark.motion
@pytest.mark.manual
@pytest.mark.danger
...
def test_agv_wheel_control(chassis, case):
    ...
    prompt_continue('确认底盘行驶路径无人、无障碍物，且急停可用。', title='轮毂运动确认')
    try:
        ...
        with allure.step("断言接口返回结果"):
            assert TuyaRobotBase.result_data(response) == 1, f'轮毂控制接口业务返回错误: {response!r}'
        duration = float(case['duration'])
        assert 0 < duration <= 5, f'轮毂控制持续时间必须在 (0, 5] 秒内，实际为 {duration!r}'
        time.sleep(duration)
    finally:
        with allure.step('调用 agv_wheel_stop 接口'):
            stop_response = chassis.agv_wheel_stop()
            assert TuyaRobotBase.result_data(stop_response) == 1, f'轮毂停止接口业务返回错误: {stop_response!r}'
