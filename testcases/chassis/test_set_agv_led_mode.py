...
cases = get_test_data_from_excel(TuyaRobotBase.CHASSIS_TEST_DATA_FILE, 'set_agv_led_mode')
...
@allure.feature('底盘')
@allure.story('底盘接口验证：set_agv_led_mode')
@pytest.mark.chassis
@pytest.mark.reset
@pytest.mark.parametrize("case", normal_cases, ids=lambda c: c["title"])
def test_set_agv_led_mode(chassis, case):
    title = case['title']
    ...
    with allure.step('调用 get_agv_led_mode 接口'):
        original = chassis.get_agv_led_mode()
        logger.debug(f"接口 get_agv_led_mode 返回：{original}")
    with allure.step("断言接口返回结果"):
        assert isinstance(original, int) and (not isinstance(original, bool))
    with allure.step("断言接口返回结果"):
        assert original in (0, 1), f'期望 0/1，实际为 {original!r}'
    try:
        with allure.step('调用 set_agv_led_mode 接口'):
            chassis.set_agv_led_mode(case['state'])
        with allure.step('调用 get_agv_led_mode 接口'):
            actual = chassis.get_agv_led_mode()
            logger.debug(f"接口 get_agv_led_mode 返回：{actual}")
        with allure.step("断言接口返回结果"):
        ...
        with allure.step('调用 set_agv_led_mode 接口'):
            chassis.set_agv_led_mode(original)
...
def test_set_agv_led_mode_exception(chassis, case):
    title = case['title']
    logger.info(f'》》》》》用例【{title}】开始测试《《《《《')
    logger.debug(f'test_api:{case["api"]}')
    logger.debug(f'state:{case["state"]}')
    with pytest.raises(TuyaRobotChassisDataException) as exc:
        with allure.step('调用 set_agv_led_mode 接口'):
            chassis.set_agv_led_mode(case['state'])
