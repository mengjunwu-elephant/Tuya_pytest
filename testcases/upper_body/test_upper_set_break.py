...
@allure.story('upper_set_break')
@pytest.mark.upper_body
@pytest.mark.manual
@pytest.mark.danger
@pytest.mark.parametrize('case', cases, ids=lambda case: case['title'])
def test_upper_set_break(upper_body, case):
    title = case['title']
    ...
    prompt_continue(case['manual_check'], title='关节抱闸安全确认')
    try:
        with allure.step('调用 upper_set_break 接口'):
            response = upper_body.upper_set_break(case['joint_id'], case['state'])
            logger.debug('接口 upper_set_break 返回：%r', response)
        assert response is not None, '关节抱闸接口未返回 ACK'
    finally:
        with allure.step('调用 upper_set_break 接口'):
            upper_body.upper_set_break(case['joint_id'], 0)
