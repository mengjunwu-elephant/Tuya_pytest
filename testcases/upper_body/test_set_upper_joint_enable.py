...
def test_set_upper_joint_enable(device, upper_body, left_arm, right_arm, case):
    ...
    prompt_continue(case["manual_check"], title=f"{target_name}关节使能安全确认")
    try:
        with allure.step(f"调用{target_name} set_upper_joint_enable 接口"):
            actual = device.result_data(target_device.set_upper_joint_enable(case["joint_id"], case["state"]))
        with allure.step("断言关节使能接口业务返回值"):
