...
cases = get_test_data_from_excel(TuyaRobotBase.UPPER_BODY_TEST_DATA_FILE, 'set_upper_movement_type')

@allure.feature('UpperBody')
@allure.story('set_upper_movement_type')
@pytest.mark.upper_body
@pytest.mark.reset
@pytest.mark.danger
@pytest.mark.parametrize('case', cases, ids=lambda case: case['title'])
def test_set_upper_movement_type(upper_body, case):
    title = case['title']
    ...
    with allure.step('调用 get_upper_movement_type 接口'):
        original = upper_body.get_upper_movement_type()
        logger.debug('接口 get_upper_movement_type 返回：%r', original)
    ...
    try:
        with allure.step('调用 set_upper_movement_type 接口'):
            upper_body.set_upper_movement_type(case['movement_type'])
        with allure.step('调用 get_upper_movement_type 接口'):
            actual = upper_body.get_upper_movement_type()
            logger.debug('接口 get_upper_movement_type 返回：%r', actual)
        ...
        with allure.step('调用 set_upper_movement_type 接口'):
            upper_body.set_upper_movement_type(left=original[0], right=original[1])
