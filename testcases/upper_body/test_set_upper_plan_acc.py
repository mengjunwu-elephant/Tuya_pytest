...
cases = get_test_data_from_excel(TuyaRobotBase.UPPER_BODY_TEST_DATA_FILE, 'set_upper_plan_acc')

@allure.feature('UpperBody')
@allure.story('set_upper_plan_acc')
@pytest.mark.upper_body
@pytest.mark.reset
@pytest.mark.danger
@pytest.mark.parametrize('case', cases, ids=lambda case: case['title'])
def test_set_upper_plan_acc(upper_body, left_arm, right_arm, case):
    title = case['title']
    ...
    with allure.step('调用 get_upper_plan_acc 接口'):
        original = upper_body.get_upper_plan_acc(mode)
        logger.debug('接口 get_upper_plan_acc 返回：%r', original)
    ...
    try:
        with allure.step('调用 set_upper_plan_acc 接口'):
            upper_body.set_upper_plan_acc(mode, case['acc'])
        with allure.step('调用 get_upper_plan_acc 接口'):
            actual = upper_body.get_upper_plan_acc(mode)
            logger.debug('接口 get_upper_plan_acc 返回：%r', actual)
        ...
        with allure.step('调用 set_upper_plan_acc 接口'):
            left_arm.set_upper_plan_acc(mode, original[0])
        with allure.step('调用 set_upper_plan_acc 接口'):
            right_arm.set_upper_plan_acc(mode, original[1])
