...
cases = get_test_data_from_excel(TuyaRobotBase.UPPER_BODY_TEST_DATA_FILE, 'get_upper_reference_frame')

@allure.feature('UpperBody')
@allure.story('get_upper_reference_frame')
@pytest.mark.upper_body
@pytest.mark.parametrize('case', cases, ids=lambda case: case['title'])
def test_get_upper_reference_frame(upper_body, case):
    title = case['title']
    logger.info(f'》》》》》用例【{title}】开始测试《《《《《')
    with allure.step('调用 get_upper_reference_frame 接口'):
        values = upper_body.get_upper_reference_frame()
        logger.debug('接口 get_upper_reference_frame 返回：%r', values)
