...
cases = get_test_data_from_excel(TuyaRobotBase.UPPER_BODY_TEST_DATA_FILE, 'set_upper_plan_sp')

@allure.feature('UpperBody')
@allure.story('set_upper_plan_sp')
@pytest.mark.upper_body
@pytest.mark.reset
@pytest.mark.danger
@pytest.mark.parametrize('case', cases, ids=lambda case: case['title'])
def test_set_upper_plan_sp(upper_body, left_arm, right_arm, case):
    title = case['title']
    ...
    mode = case['mode']
    with allure.step('调用 get_upper_plan_sp 接口'):
        original = upper_body.get_upper_plan_sp(mode)
        logger.debug('接口 get_upper_plan_sp 返回：%r', original)
    ...
    try:
        with allure.step('调用 set_upper_plan_sp 接口'):
            upper_body.set_upper_plan_sp(mode, case['speed'])
        with allure.step('调用 get_upper_plan_sp 接口'):
            actual = upper_body.get_upper_plan_sp(mode)
            logger.debug('接口 get_upper_plan_sp 返回：%r', actual)
        ...
        with allure.step('调用 set_upper_plan_sp 接口'):
            left_arm.set_upper_plan_sp(mode, original[0])
        with allure.step('调用 set_upper_plan_sp 接口'):
            right_arm.set_upper_plan_sp(mode, original[1])
