...
@allure.story("验证单臂和双臂 upper_drag_teach_record_pause 接口")
@pytest.mark.upper_body
@pytest.mark.manual
@pytest.mark.motion
@pytest.mark.danger
@pytest.mark.parametrize("case", cases, ids=lambda c: c["title"])
def test_upper_drag_teach_record_pause(device, upper_body, left_arm, right_arm, case):
    ...
    try:
        with allure.step(f"启动{target_name}拖动示教录制"):
            device.result_data(target_device.upper_drag_teach_record())
        time.sleep(case["duration"])
    finally:
        with allure.step(f"调用{target_name} upper_drag_teach_record_pause 接口"):
            actual = device.result_data(target_device.upper_drag_teach_record_pause())
