...
cases = get_test_data_from_excel(TuyaRobotBase.UPPER_BODY_TEST_DATA_FILE, 'get_coords')

@allure.feature('UpperBody')
@allure.story('get_coords')
@pytest.mark.upper_body
@pytest.mark.parametrize('case', cases, ids=lambda case: case['title'])
def test_get_coords(upper_body, case):
    title = case['title']
    logger.info(f'》》》》》用例【{title}】开始测试《《《《《')
    with allure.step('调用 get_coords 接口'):
        response_1 = upper_body.get_coords()
        logger.debug('接口 get_coords 返回：%r', response_1)
