...
def restore_soft_joint_limits(device, upper_body):
    yield
    with allure.step("关节最小角度设置全部用例结束后恢复J1到J7软件限位"):
...
@allure.story("设置上半身关节最小角度")
@pytest.mark.upper_body
@pytest.mark.danger
@pytest.mark.reset
@pytest.mark.parametrize("case", normal_cases, ids=lambda c: c["title"])
def test_set_joint_min_angle(device, upper_body, case):
    title = case["title"]
    ...
    try:
        with allure.step("调用 set_joint_min_angle 接口"):
            actual = device.result_data(upper_body.set_joint_min_angle(joint_id, degree))
        ...
        with allure.step("回读并断言目标关节最小角度"):
            limits = device.result_data(upper_body.get_upper_joints_min_angle())
            assert isinstance(limits, (list, tuple)) and len(limits) == 8
            actual_degree = limits[joint_id - 1]
            allure.attach(str(degree), name="期望最小角度", attachment_type=allure.attachment_type.TEXT)
            allure.attach(str(actual_degree), name="实际最小角度", attachment_type=allure.attachment_type.TEXT)
...
@allure.feature("上半身参数设置")
@allure.story("验证关节最小角度非法参数")
@pytest.mark.upper_body
@pytest.mark.parametrize("case", exception_cases, ids=lambda c: c["title"])
def test_set_joint_min_angle_exception(upper_body, case):
    title = case["title"]
    joint_id = int(case["joint_id"])
    degree = float(case["degree"])
    ...
    with pytest.raises(TuyaRobotDualArmDataException) as exc:
        with allure.step("调用 set_joint_min_angle 接口并验证非法参数"):
            upper_body.set_joint_min_angle(joint_id, degree)
