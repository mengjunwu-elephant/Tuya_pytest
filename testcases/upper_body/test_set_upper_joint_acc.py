...
def test_set_upper_joint_acc(device, upper_body, left_arm, right_arm, case):
    ...
    with allure.step("读取左右臂原始关节加速度"):
        original_left_data = device.result_data(left_arm.get_upper_joint_acc(case["joint_id"]))
        original_right_data = device.result_data(right_arm.get_upper_joint_acc(case["joint_id"]))
    ...
    try:
        with allure.step(f"调用{target_name} set_upper_joint_acc 接口"):
            if target == "both":
                result = upper_body.set_upper_joint_acc(case["joint_id"], case["acc"])
            else:
                result = target_device.set_upper_joint_acc(case["joint_id"], case["acc"])
        ...
        with allure.step("分别回读左右臂关节加速度"):
            current_left_data = device.result_data(left_arm.get_upper_joint_acc(case["joint_id"]))
            current_right_data = device.result_data(right_arm.get_upper_joint_acc(case["joint_id"]))
        ...
        with allure.step("分别恢复左右臂原始关节加速度"):
            device.result_data(left_arm.set_upper_joint_acc(case["joint_id"], original_left))
            device.result_data(right_arm.set_upper_joint_acc(case["joint_id"], original_right))
