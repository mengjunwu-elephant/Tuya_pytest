...
@allure.story("暂停单臂和双臂运动")
@pytest.mark.upper_body
@pytest.mark.parametrize("case", cases, ids=lambda c: c["title"])
def test_upper_pause(device, upper_body, left_arm, right_arm, case):
    ...
    try:
        with allure.step(f"调用{target_name} upper_pause 接口"):
            actual = device.result_data(target_device.upper_pause())
        with allure.step("断言暂停接口业务返回值"):
        ...
        with allure.step(f"恢复{target_name}运动状态"):
