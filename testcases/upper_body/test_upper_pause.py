...
cases = get_test_data_from_excel(TuyaRobotBase.UPPER_BODY_TEST_DATA_FILE, 'upper_pause')

@allure.feature('UpperBody')
@allure.story('upper_pause')
@pytest.mark.upper_body
@pytest.mark.parametrize('case', cases, ids=lambda case: case['title'])
def test_upper_pause(upper_body, case):
    title = case['title']
    ...
    try:
        with allure.step('调用 upper_pause 接口'):
            upper_body.upper_pause()
        with allure.step('调用 get_upper_is_paused 接口'):
            states = upper_body.get_upper_is_paused()
            logger.debug('接口 get_upper_is_paused 返回：%r', states)
        ...
        with allure.step('调用 upper_resume 接口'):
            upper_body.upper_resume()
