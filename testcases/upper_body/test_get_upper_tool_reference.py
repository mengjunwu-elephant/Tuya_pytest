...
@allure.story("查询单臂和双臂工具参考系")
@pytest.mark.upper_body
@pytest.mark.parametrize("case", cases, ids=lambda c: c["title"])
def test_get_upper_tool_reference(device, upper_body, left_arm, right_arm, case):
    title = case["title"]
    target = case["target"]
    targets = {
        "left": (left_arm, "左臂"),
    ...
    with allure.step(f"调用{target_name} get_upper_tool_reference 接口"):
        result = target_device.get_upper_tool_reference()
        actual = device.result_data(result)
        logger.debug(f"接口 get_upper_tool_reference 返回：{actual}")

    with allure.step(f"断言{target_name}工具参考系返回结构"):
        if target == "both":
            ...
            for arm_data in (actual["left"], actual["right"]):
                ...
                assert len(arm_data) == 6, f"单侧数据长度错误：{arm_data!r}"
            ...
            assert len(actual) == 6, f"单臂返回长度错误：{actual!r}"
