...
@allure.story("查询单臂和双臂关节模型方向")
@pytest.mark.upper_body
@pytest.mark.parametrize("case", cases, ids=lambda c: c["title"])
def test_get_upper_model_direction(device, upper_body, left_arm, right_arm, case):
    title = case["title"]
    target = case["target"]
    targets = {
        "left": (left_arm, "左臂"),
    ...
    with allure.step(f"调用{target_name} get_upper_model_direction 接口"):
        result = target_device.get_upper_model_direction()
        actual = device.result_data(result)
        logger.debug(f"接口 get_upper_model_direction 返回：{actual}")

    with allure.step(f"断言{target_name}关节模型方向返回结构"):
        if target == "both":
            assert isinstance(actual, dict), f"双臂返回类型错误：{actual!r}"
            assert set(actual) >= {"left", "right"}, f"双臂返回缺少 left/right：{actual!r}"
            assert all(isinstance(actual[side], (list, tuple)) for side in ("left", "right"))
        else:
