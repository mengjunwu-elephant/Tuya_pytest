...
@allure.story("查询上半身关节最小角度")
@pytest.mark.upper_body
@pytest.mark.parametrize("case", normal_cases, ids=lambda c: c["title"])
def test_get_upper_joints_min_angle(device, upper_body, case):
    title = case["title"]
    expected = [
        TuyaRobotBase.UPPER_BODY_JOINT_SOFT_LIMITS[joint_id][0]
    ...
    with allure.step("调用 get_upper_joints_min_angle 接口"):
        actual = device.result_data(upper_body.get_upper_joints_min_angle())
    ...
    with allure.step("断言J1到J7等于软件下限"):
        allure.attach(str(expected), name="期望J1到J7软件下限", attachment_type=allure.attachment_type.TEXT)
        allure.attach(str(list(actual[:7])), name="实际J1到J7最小角度", attachment_type=allure.attachment_type.TEXT)
