...
cases = get_test_data_from_excel(TuyaRobotBase.UPPER_BODY_TEST_DATA_FILE, 'upper_drag_teach_record')

@allure.feature('UpperBody')
@allure.story('upper_drag_teach_record')
@pytest.mark.upper_body
@pytest.mark.manual
@pytest.mark.motion
@pytest.mark.danger
@pytest.mark.parametrize('case', cases, ids=lambda case: case['title'])
def test_upper_drag_teach_record(upper_body, case):
    title = case['title']
    ...
    prompt_continue(case['manual_check'], title='拖动示教录制')
    try:
        with allure.step('调用 upper_drag_teach_record 接口'):
            response = upper_body.upper_drag_teach_record()
            logger.debug('接口 upper_drag_teach_record 返回：%r', response)
        assert response is not None, '拖动示教录制接口未返回 ACK'
