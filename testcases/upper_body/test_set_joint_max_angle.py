...
cases = get_test_data_from_excel(TuyaRobotBase.UPPER_BODY_TEST_DATA_FILE, 'set_joint_max_angle')

@allure.feature('UpperBody')
@allure.story('set_joint_max_angle')
@pytest.mark.upper_body
@pytest.mark.danger
@pytest.mark.parametrize('case', cases, ids=lambda case: case['title'])
def test_set_joint_max_angle(upper_body, left_arm, right_arm, case):
    title = case['title']
    ...
    with allure.step('调用 get_upper_joints_max_angle 接口'):
        original = upper_body.get_upper_joints_max_angle()
        logger.debug('接口 get_upper_joints_max_angle 返回：%r', original)
    ...
    try:
        with allure.step('调用 set_joint_max_angle 接口'):
            upper_body.set_joint_max_angle(joint_id, case['degree'], persist=False)
        with allure.step('调用 get_upper_joints_max_angle 接口'):
            actual = upper_body.get_upper_joints_max_angle()
            logger.debug('接口 get_upper_joints_max_angle 返回：%r', actual)
        ...
        with allure.step('调用 set_joint_max_angle 接口'):
            left_arm.set_joint_max_angle(joint_id, original['left'][joint_id - 1], persist=False)
        with allure.step('调用 set_joint_max_angle 接口'):
            right_arm.set_joint_max_angle(joint_id, original['right'][joint_id - 1], persist=False)
