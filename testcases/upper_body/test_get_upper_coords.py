...
@allure.story("查询单臂和双臂末端坐标")
@pytest.mark.upper_body
@pytest.mark.parametrize("case", cases, ids=lambda c: c["title"])
def test_get_upper_coords(device, upper_body, left_arm, right_arm, case):
    title = case["title"]
    target = case["target"]
    targets = {
        "left": (left_arm, "左臂"),
    ...
    with allure.step(f"调用{target_name} get_upper_coords 接口"):
        result = target_device.get_upper_coords()
        actual = device.result_data(result)
        logger.debug(f"接口 get_upper_coords 返回：{actual}")

    with allure.step(f"断言{target_name}末端坐标返回结构"):
        if target == "both":
