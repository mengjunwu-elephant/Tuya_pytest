...
def test_set_vr_mode(device, upper_body, left_arm, right_arm, case):
    ...
    with allure.step("读取左右臂原始VR模式"):
        original_left_data = device.result_data(left_arm.get_upper_vr_mode())
        original_right_data = device.result_data(right_arm.get_upper_vr_mode())
        original_left = original_left_data
        original_right = original_right_data
    try:
        with allure.step(f"调用{target_name} set_upper_vr_mode 接口"):
            if target == "both":
                result = upper_body.set_upper_vr_mode(case["mode"])
            else:
                result = target_device.set_upper_vr_mode(case["mode"])
        ...
        with allure.step("分别回读左右臂VR模式"):
            current_left_data = device.result_data(left_arm.get_upper_vr_mode())
            current_right_data = device.result_data(right_arm.get_upper_vr_mode())
        ...
        with allure.step("分别恢复左右臂原始VR模式"):
            device.result_data(left_arm.set_upper_vr_mode(original_left))
            device.result_data(right_arm.set_upper_vr_mode(original_right))
