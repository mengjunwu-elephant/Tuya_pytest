...
cases = get_test_data_from_excel(TuyaRobotBase.UPPER_BODY_TEST_DATA_FILE, 'get_upper_joints_max_angle')

@allure.feature('UpperBody')
@allure.story('get_upper_joints_max_angle')
@pytest.mark.upper_body
@pytest.mark.parametrize('case', cases, ids=lambda case: case['title'])
def test_get_upper_joints_max_angle(upper_body, case):
    title = case['title']
    logger.info(f'》》》》》用例【{title}】开始测试《《《《《')
    with allure.step('调用 get_upper_joints_max_angle 接口'):
        response_1 = upper_body.get_upper_joints_max_angle()
        logger.debug('接口 get_upper_joints_max_angle 返回：%r', response_1)
