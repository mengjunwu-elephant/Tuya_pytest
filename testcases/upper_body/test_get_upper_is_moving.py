...
cases = get_test_data_from_excel(TuyaRobotBase.UPPER_BODY_TEST_DATA_FILE, 'get_upper_is_moving')

@allure.feature('UpperBody')
@allure.story('get_upper_is_moving')
@pytest.mark.upper_body
@pytest.mark.parametrize('case', cases, ids=lambda case: case['title'])
def test_get_upper_is_moving(upper_body, case):
    title = case['title']
    ...
    assert all((state in (0, 1, False, True) for state in states))
