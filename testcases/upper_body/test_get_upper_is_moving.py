...
def test_get_upper_is_moving(device, upper_body, left_arm, right_arm, case):
    ...
    with allure.step(f"调用{target_name} get_upper_is_moving 接口"):
        result = target_device.get_upper_is_moving()
        actual = device.result_data(result)
        logger.debug(f"接口 get_upper_is_moving 返回：{actual}")

    with allure.step(f"断言{target_name}运动状态返回结构"):
        if target == "both":
