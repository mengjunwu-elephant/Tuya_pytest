...
cases = get_test_data_from_excel(TuyaRobotBase.HEAD_TEST_DATA_FILE, "get_head_robot_status")

@allure.feature("头部 PI4")
@allure.story("get_head_robot_status")
@pytest.mark.head
@pytest.mark.parametrize("case", cases, ids=lambda c: c["title"])
def test_get_head_robot_status(head, case):
    logger.info("》》》》》用例【%s】开始测试《《《《《", case["title"])
    logger.debug("test_api:%s", case["api"])
    with allure.step("调用 get_head_robot_status 接口"):
        actual = head.get_head_robot_status()
